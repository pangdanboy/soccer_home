...
router.get('/systemInfo', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  const findArr = []
  // 查询今日比赛数量和参赛人数
  const nowDateMatchCount = Match.find({ matchDate: nowDate }).count()
  const nowDateMatchGamerList = Match.find({ matchDate: nowDate }, { matchGamerList: 1 })
  // 查询比赛总数量和总参赛人数
  ...
  const systemInfo = {}
  // 计算今天是这周的第几天
  const weekOfDay = DATE[parseInt(moment(nowDate).format('E'))]
  // 当天日期与所处周的第一天
  systemInfo.nowDate = moment(nowDate).format('YYYY-MM-DD')
